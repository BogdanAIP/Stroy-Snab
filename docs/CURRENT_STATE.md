...
6. checksum-aware поиск unlabeled ИНН/ОГРН, включая форматированные пробелами/дефисами представления, плюс базовые реквизиты/email/phone/URL и private denylist;
...
Дополнительно перед финальным frozen HEAD усилена checksum-проверка ИНН/ОГРН: форматирующие пробелы, NBSP, thin space и дефисы нормализуются только внутри кандидата идентификатора, без глобального склеивания числовых фрагментов текста. Добавлены регрессии для корректного 12-значного ИНН и форматированных ИНН/ОГРН.
