...
| `UNTRUSTED_DOCUMENT_PARSER` | входной PDF/XLSX/JPG от внешнего контрагента может эксплуатировать parser bug или вызвать resource exhaustion до обезличивания | считать входы недоверенными; byte/page/pixel/ZIP limits сейчас; до production — отдельный bounded worker/process, no-network/least-privilege и timeout/memory limits для native parsers |
