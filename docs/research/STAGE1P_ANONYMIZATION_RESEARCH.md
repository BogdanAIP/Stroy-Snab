...
```text
CASE_0001/
  manifest.json        # safe role/type/provenance only
  visual/page-001.png  # sanitized rendered appearance where layout matters
  document.xlsx        # optional rebuilt XLSX when native cell structure matters
  expected.json        # public gold, no reverse identifiers
```

This separates visual/layout evidence from semantic gold and avoids carrying unknown source-container internals into Git.
...
4. Public case IDs must not encode source filenames, dates, supplier identity or original document numbers.
...
3. Implement PDF/image visual derivative prototype and PDF/container checks.
