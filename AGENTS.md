...
Репозиторий публичный. Сырые пользовательские УПД, счета, договоры, банковские/налоговые реквизиты, персональные данные, закрытые коммерческие цены и иные реальные документы не коммитятся.

Реальные документы из ChatGPT Library могут использоваться как private evaluation corpus. В репозиторий попадают только обезличенные/синтетические fixtures, схемы, manifest без секретных данных и ожидаемые результаты, разрешённые `docs/DATA_POLICY.md`.
