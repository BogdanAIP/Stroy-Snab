...
_IDENTIFIER_TOKEN_RE = re.compile(r"(?<!\d)\d(?:[ \t\u00a0\u2009-]*\d){9,14}(?!\d)")
_IDENTIFIER_SEPARATOR_RE = re.compile(r"[ \t\u00a0\u2009-]+")


def _normalize_identifier_digits(value: str) -> str:
    """Remove formatting separators from a candidate tax identifier."""
    return _IDENTIFIER_SEPARATOR_RE.sub("", value)
...
def is_inn(value: str) -> bool:
    value = _normalize_identifier_digits(value)
