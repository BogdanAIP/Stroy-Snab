from __future__ import annotations

from pathlib import Path
import os
import tempfile
from typing import Any, Iterable

import xlsxwriter

from .leakcheck import scan_path, scan_text

_XLSX_MAX_ROWS = 1_048_576
_XLSX_MAX_COLUMNS = 16_384


def _validate_sheet_name(name: str, forbidden_tokens: Iterable[str]) -> str:
    if not isinstance(name, str) or not name.strip():
        raise ValueError("sheet name must be non-empty")
    if len(name) > 31 or any(char in name for char in "[]:*?/\\"):
        raise ValueError("invalid XLSX sheet name")
    findings = scan_text(name, location="sheet-name", forbidden_tokens=forbidden_tokens)
    if findings:
        raise ValueError("sheet name contains data blocked by leak policy")
    return name


def _validate_cell_value(value: Any, forbidden_tokens: Iterable[str]) -> Any:
    if value is None or isinstance(value, (bool, int, float)):
        return value
    if not isinstance(value, str):
        raise TypeError(f"unsupported sanitized cell type: {type(value).__name__}")
    findings = scan_text(value, location="cell", forbidden_tokens=forbidden_tokens)
    if findings:
        raise ValueError("sanitized cell contains data blocked by leak policy")
    return value


def _freeze_forbidden_tokens(forbidden_tokens: Iterable[str]) -> tuple[str, ...]:
    tokens = tuple(forbidden_tokens)
    if not all(isinstance(token, str) for token in tokens):
        raise TypeError("forbidden_tokens must contain strings only")
    return tokens


def _row_cell(rows: list[list[Any]], row: int, col: int) -> Any:
    if row >= len(rows) or col >= len(rows[row]):
        return None
    return rows[row][col]


def _assert_merge_does_not_discard_values(
    rows: list[list[Any]],
    first_row: int,
    first_col: int,
    last_row: int,
    last_col: int,
) -> None:
    for row in range(first_row, last_row + 1):
        for col in range(first_col, last_col + 1):
            if row == first_row and col == first_col:
                continue
            if _row_cell(rows, row, col) not in (None, ""):
                raise ValueError("merge would discard a non-empty sanitized cell")


def write_sanitized_workbook(
    output_path: str | Path,
    *,
    sheets: list[dict[str, Any]],
    forbidden_tokens: Iterable[str],
) -> Path:
    """Create a new XLSX from an explicitly sanitized allowlist representation.

    This function intentionally never opens or copies an original workbook.
    Callers must first map private input into allowed sheet names, values, widths
    and merge ranges. Formula and URL auto-detection are disabled so private or
    active content cannot be smuggled through a string value. The rebuilt
    container is independently leak-checked before it becomes the final output.
    Any XLSX writer condition that would truncate or silently drop a value is
    treated as a failure.
    """
    if not sheets:
        raise ValueError("at least one sanitized sheet is required")
    tokens = _freeze_forbidden_tokens(forbidden_tokens)
    output = Path(output_path)
    if output.exists():
        raise FileExistsError("sanitized workbook output must not already exist")
    output.parent.mkdir(parents=True, exist_ok=True)
    fd, temp_name = tempfile.mkstemp(prefix=f".{output.name}.", suffix=".tmp.xlsx", dir=output.parent)
    os.close(fd)
    temp_path = Path(temp_name)

    workbook = xlsxwriter.Workbook(
        temp_path,
        {
            "strings_to_formulas": False,
            "strings_to_urls": False,
            "strings_to_numbers": False,
        },
    )
    workbook.set_properties(
        {
            "title": "Stroy-Snab anonymized fixture",
            "subject": "anonymized-real evaluation derivative",
            "author": "Stroy-Snab",
            "manager": "",
            "company": "",
            "category": "evaluation fixture",
            "keywords": "",
            "comments": "No reverse mapping to private source is stored in this file.",
        }
    )

    try:
        for sheet_spec in sheets:
            unknown = set(sheet_spec) - {"name", "rows", "column_widths", "merges"}
            if unknown:
                raise ValueError("unknown sanitized sheet fields present")
            name = _validate_sheet_name(sheet_spec.get("name", ""), tokens)
            rows = sheet_spec.get("rows")
            if not isinstance(rows, list):
                raise ValueError("rows must be a list")
            if len(rows) > _XLSX_MAX_ROWS:
                raise ValueError("sanitized sheet exceeds XLSX row limit")
            worksheet = workbook.add_worksheet(name)

            widths = sheet_spec.get("column_widths", {})
            if not isinstance(widths, dict):
                raise ValueError("column_widths must be an object")
            for col, width in widths.items():
                if (
                    not isinstance(col, int)
                    or not 0 <= col < _XLSX_MAX_COLUMNS
                    or not isinstance(width, (int, float))
                ):
                    raise ValueError("column_widths contain an invalid column or width")
                result = worksheet.set_column(col, col, min(float(width), 80.0))
                if result != 0:
                    raise ValueError("XLSX writer rejected a column width")

            for row_idx, row in enumerate(rows):
                if not isinstance(row, list):
                    raise ValueError("each row must be a list")
                if len(row) > _XLSX_MAX_COLUMNS:
                    raise ValueError("sanitized row exceeds XLSX column limit")
                for col_idx, raw_value in enumerate(row):
                    value = _validate_cell_value(raw_value, tokens)
                    result = worksheet.write(row_idx, col_idx, value)
                    if result != 0:
                        raise ValueError("XLSX writer would truncate or drop a sanitized cell")

            merges = sheet_spec.get("merges", [])
            if not isinstance(merges, list):
                raise ValueError("merges must be a list")
            for merge in merges:
                if not (isinstance(merge, dict) and set(merge) == {"first_row", "first_col", "last_row", "last_col"}):
                    raise ValueError("merge must contain first_row/first_col/last_row/last_col")
                first_row, first_col, last_row, last_col = [
                    merge[key] for key in ("first_row", "first_col", "last_row", "last_col")
                ]
                if not all(isinstance(value, int) for value in (first_row, first_col, last_row, last_col)):
                    raise ValueError("merge coordinates must be integers")
                if not (
                    0 <= first_row <= last_row < _XLSX_MAX_ROWS
                    and 0 <= first_col <= last_col < _XLSX_MAX_COLUMNS
                ):
                    raise ValueError("merge coordinates are outside XLSX bounds")
                if first_row == last_row and first_col == last_col:
                    raise ValueError("single-cell merge is not meaningful")

                _assert_merge_does_not_discard_values(rows, first_row, first_col, last_row, last_col)
                merged_value = _validate_cell_value(_row_cell(rows, first_row, first_col), tokens)
                result = worksheet.merge_range(first_row, first_col, last_row, last_col, merged_value)
                if result != 0:
                    raise ValueError("XLSX writer rejected a merge range")
    except Exception:
        try:
            workbook.close()
        finally:
            temp_path.unlink(missing_ok=True)
        raise
    else:
        try:
            workbook.close()
            report = scan_path(temp_path, forbidden_tokens=tokens)
            if not report.passed:
                raise ValueError("rebuilt XLSX failed independent leak check")
            os.replace(temp_path, output)
        except Exception:
            temp_path.unlink(missing_ok=True)
            raise
    return output
